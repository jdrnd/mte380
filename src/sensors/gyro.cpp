...
void Gyro::read(){
    ...
    roll = roll + ((GyX - x_offset) * dt / 1000000.)/130.1777777;//130.17777777 IS SCALING CONST TO GET RAD ANGULAR DATA TO DEGREES
    pitch = pitch + ((GyY - y_offset) * dt / 1000000.)/130.1777777;
    yaw = yaw + ((GyZ - z_offset) * dt / 1000000.)/130.1777777;
    
    //DELAY SO YAW, PITCH, AND ROLL UPDATE DOESN'T GO OUTSIDE OF RANGE REPRESENTABLE BY THE DATATYPE USED?
    delay(10);
}
